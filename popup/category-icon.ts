// Popup-side icon rendering: inline SVG (the popup is vanilla DOM, so no
...
// Inline SVG for a category — honours an explicit icon, else the name guess.
export function categorySvg(category: { name: string; icon?: IconKey }): string {
  return iconSvg(resolveCategoryIcon(category))
}
