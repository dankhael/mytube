...
export default function AddCategoryModal({ existing, onClose, onSubmit }: Props) {
  ...
  return (
    <ModalShell title={existing ? tr('modal.editCategory') : tr('modal.newCategory')} onClose={onClose}>
      <label className="mb-2 block text-sm text-yt-muted">{tr('modal.name')}</label>
      <input
        autoFocus
        value={name}
      ...
      <label className="mb-2 mt-4 block text-sm text-yt-muted">{tr('modal.icon')}</label>
      ...
      <div className="mt-6 flex justify-end gap-2">
        ...
        >
          {existing ? tr('common.save') : tr('common.create')}
        </button>
      </div>
    </ModalShell>
  )
